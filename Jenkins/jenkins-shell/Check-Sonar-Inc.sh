...
if [[ "${SONARCLASS}" == "FULL" ]];then
    ...
    if [[ $PREHASH == "" ]];then
        ...
        cd -
        echo $HASH > /jenkinsdata/data/.jenkins/common-extended/$systemname-HASH.txt
        curl -s -X POST  -u 'admin:zjxl2018#' http://$IPADDRESS:9000/api/users/create -d "login=$PROJECTNAME&name=$PROJECTNAME&password=$PROJECTNAME"
        curl -s -X POST  -u 'admin:zjxl2018#' http://$IPADDRESS:9000/api/permissions/add_user -d "login=$PROJECTNAME&permission=user&projectKey=$systemname"
        curl -s -X POST  -u 'admin:zjxl2018#' http://$IPADDRESS:9000/api/permissions/add_user -d "login=$PROJECTNAME&permission=codeviewer&projectKey=$systemname"
...
do
        ...
        date
        sleep 10
        RESON=`curl -s -X POST  -u 'admin:zjxl2018#' http://$IPADDRESS:9000/api/ce/activity -d "component=$systemname&status=IN_PROGRESS,PENDING" | grep $systemname`
        #echo $RESON
done
